...
assert.equal([...resources].filter(p=>/^\/images\/actual-gallery\//.test(p)).length,9);
...
console.log(`Page checks passed: ${ids.length} IDs, ${resources.size} image resources, ${(total/1024/1024).toFixed(2)} MiB including responsive alternatives. 9 photos + 15 renders.`);
